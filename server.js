...
function registerTools(s) {
 ...
 : freq > 3 ? " УСТАЛОСТЬ (freq > 3)"
 : ctr < 0.5 && spend > 3 ? " НИЗКИЙ CTR"
 ...
 адсеты: alerts.map((a) => ({ ...a, рекомендация: a.frequency > 4 ? " срочно расширить аудиторию или выключить" : " следить" })),
 }, null, 2),
 }],
 };
 }
 ...
 if (spend > 2 && ctr < min_ctr) alerts.push({ тип: " НИЗКИЙ CTR", объект: "адсет", name: a.name, детали: `CTR ${ctr.toFixed(2)}% < ${min_ctr}%` });
 if (freq > max_frequency) alerts.push({ тип: " ВЫСОКАЯ ЧАСТОТА", объект: "адсет", name: a.name, детали: `Frequency ${freq.toFixed(2)} > ${max_frequency}` });
 }
 ...
 итог: toKill.length > 0 ? ` ${toKill.length} объявлений нужно выключить` : " Всё в порядке",
 }, null, 2),
 }],
 };
 }
 ...
 return { content: [{ type: "text", text: JSON.stringify({ статус: " ВСЕ КАМПАНИИ ОСТАНОВЛЕНЫ", остановлено: results.length, кампании: results }, null, 2) }] };
 }
 );
